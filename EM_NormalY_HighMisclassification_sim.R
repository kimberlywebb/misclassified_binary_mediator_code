...
for(i in 1:n_sim){
  ...
  mediation_results <- mediation_EM_normalY(Mstar = data[["obs_mediator"]],
...
summarized_sim_results <- sim_results %>%
  ...
  dplyr::summarise(mean_estimate = mean(estimate),
                   ...
                   bias = mean(bias)) %>%
  ungroup()  %>%
  mutate(true_value = c(1, -2, .5, 1, -.5, 1, -1.5, 1, 1, -.2, -2, 1.5))
...
save(sim_results, summarized_sim_results,
     file = paste0(save_directory, "EM_NormalY_HighMisclassification_results.RData"))
