mediation_EM_normalY <- function(Mstar, # Observed mediator vector
  ...
  estimates_i <- if ((pistar_11 > .50 | pistar_22 > .50) |
    ...
    c(-1*turboEM::pars(results)[1:(ncol(X) + 1)], turboEM::pars(results)[gamma_flip_index],
      ...
      turboEM::pars(results)[11], turboEM::pars(results)[12])
  }
  ...
  theta_param_names <- c("theta_0", "theta_x", "theta_m", "theta_c", "sigma")
